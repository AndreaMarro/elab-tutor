...
const CircuitStatusChip = ({ status }) => {
  ...
  const config = {
    ok: { icon: '✓', label: 'Circuito OK', bg: 'var(--color-accent-light, #E8F5E9)', color: 'var(--color-success, #2E7D32)', border: 'var(--color-accent, var(--elab-lime))' },
    ...
    error: { icon: '✗', label: `${status.errors.length} errore${status.errors.length > 1 ? 'i' : ''}`, bg: 'var(--color-danger-light, #FFEBEE)', color: 'var(--color-danger, #C62828)', border: 'var(--color-vol3, var(--elab-red))' },
  };
