  ...
  return (
    <g transform={`translate(${x}, ${y})`} data-component-id={id} data-type="multimeter" role="img"
      ...
      <text x="-2" y="-12" textAnchor="middle" fontSize="8"
        fill="var(--color-accent, #7CB342)" fontFamily="Fira Code, monospace" fontWeight="bold">
        {displayValue}
      </text>
      <text x="12" y="-12" fontSize="5" fill="var(--color-accent, #7CB342)"
      ...
      {['V', 'Ω', 'A'].map((label, i) => {
        ...
        return (
          <text key={label}
            ...
            fill={isActive ? 'var(--color-accent, #7CB342)' : '#555'}
      ...
      >
        ...
        {probeSnapped?.negative && (
          <circle cx={negProbe.x} cy={negProbe.y} r="8" fill="var(--color-accent, #7CB342)" opacity="0.3">
      ...
      >
        ...
        {probeSnapped?.positive && (
          <circle cx={posProbe.x} cy={posProbe.y} r="8" fill="var(--color-accent, #7CB342)" opacity="0.3">
            <animate attributeName="r" values="8;11;8" dur="1s" repeatCount="indefinite" />
          </circle>
        )}
