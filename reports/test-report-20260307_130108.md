...
======================================================================
  ...
  Model: galileo-brain
  Date: 2026-03-07 13:01:08
======================================================================

  OVERALL: 25/120 (20.8%) — Grade: F
  ...
  METRICS:
  Intent accuracy:       73/120 (60.8%)
  Action accuracy:       48/120 (40.0%)
  needs_llm accuracy:    96/120 (80.0%)
  Response non-empty:   120/120 (100.0%)
  Hint non-empty:       120/120 (100.0%)

  LATENCY:
  Min: 2746ms | P50: 3751ms | P95: 6222ms | Max: 33480ms
  ...
  E027 [circuit/multi_component]:
    ...
    Reason:  INTENT:action!=circuit, ACTIONS:['[AZIONE:led1]', '[AZIONE:resistor1]', '[AZIONE:button1]']!=['[INTENT:*]']
    Got:     intent=action, actions=['[AZIONE:led1]', '[AZIONE:resistor1]', '[AZIONE:button1]'], needs_llm=False
  ...
  E042 [navigation/openvolume]:
    ...
    Reason:  ACTIONS:['[NAVIGAZIONE:volume2]']!=['[AZIONE:openvolume:2]']
    Got:     intent=navigation, actions=['[NAVIGAZIONE:volume2]'], needs_llm=False
  ...
  E058 [action/measure]:
    ...
    Reason:  INTENT:code!=action, ACTIONS:[]!=['[AZIONE:measure:*]'], NEEDS_LLM:True!=False
    Got:     intent=code, actions=[], needs_llm=True
  E059 [action/measure]:
    Input:   Misura la corrente nel circuito
    Reason:  ACTIONS:['[AZIONE:current]']!=['[AZIONE:measure:*]']
    Got:     intent=action, actions=['[AZIONE:current]'], needs_llm=False
  E060 [action/setvalue]:
    Input:   Cambia il resistore a 470 ohm
    Reason:  INTENT:code!=action, ACTIONS:['[AZIONE:modifica-resistor-valore resistor1 470]']!=['[AZIONE:setvalue:*]']
    Got:     intent=code, actions=['[AZIONE:modifica-resistor-valore resistor1 470]'], needs_llm=False
  ...
  E063 [action/movecomponent]:
    ...
    Reason:  INTENT:navigation!=action, ACTIONS:['[AZIONE:resistor1-move-to-top]']!=['[AZIONE:movecomponent:*]']
    Got:     intent=navigation, actions=['[AZIONE:resistor1-move-to-top]'], needs_llm=False
  ...
  E091 [vision/implicit]:
    ...
    Reason:  INTENT:code!=vision, NEEDS_LLM:False!=True
    Got:     intent=code, actions=['[AZIONE:check-circuit]'], needs_llm=False
  E092 [vision/debug]:
  ...
  E111 [circuit/wiring]:
    ...
    Reason:  INTENT:action!=circuit, ACTIONS:['[AZIONE:connectBus(pot1, W_A0, bus-)]']!=['[AZIONE:addwire:*]']
    Got:     intent=action, actions=['[AZIONE:connectBus(pot1, W_A0, bus-)]'], needs_llm=False
  E112 [action/play]:
    Input:   Vai!
  ...
  E119 [code/setcode]:
    ...
    Got:     intent=code, actions=['[AZIONE:codice]'], needs_llm=True
  ...
  action: 7% — needs 38 more correct
    Failure breakdown: {'ACTIONS': 37, 'INTENT': 15, 'NEEDS_LLM': 7}
  vision: 18% — needs 9 more correct
    Failure breakdown: {'NEEDS_LLM': 7, 'INTENT': 3}
  ...
  RECOMMENDATIONS:
  ------------------------------------------------------------
  [HIGH] Intent confusion matrix: {'?->navigation': 10, '?->code': 10, '?->tutor': 4, '?->action': 22, '?->vision': 1}
  ...
  [MEDIUM] Average latency 4287ms is high — consider smaller quantization or GPU
