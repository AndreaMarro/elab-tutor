...
await page.getByRole('button', { name: /^simulatore$/i }).first().click({ force: true });
...
const data = await page.evaluate(() => {
  ...
  return {
    ...
    chatTextarea: !!document.querySelector('textarea[placeholder="Chiedi a Galileo..."]'),
  };
});

console.log(JSON.stringify(data, null, 2));
await page.screenshot({ path: '/tmp/session61_probe_simulator.png', fullPage: true });
await browser.close();
